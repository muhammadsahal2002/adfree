...
def get_session_token():
    ...
    try:
        ...
        log_message("Fresh cookies obtained")
    except Exception as e:
    ...
    if not t_hash_t_encoded:
        # Use addhash but ensure it has the right format
        t_hash_t_encoded = addhash_final
        # Check if it has the mobile suffix
    ...
    # Fix: If the token has ::su, replace with ::db (or keep as-is)
    # The script will keep whatever the site returns
    log_message(f"Token generated: {t_hash_t_raw[:50]}...")

    # Return only the three required fields (no generated_at)
    return {
        "token": t_hash_t_raw,
